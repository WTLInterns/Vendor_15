...
const Sidebar = () => {
  ...
  return (
    <div className="w-64 h-screen bg-white shadow-lg fixed left-0 top-0 z-50 border-r transition-transform transform md:translate-x-0 -translate-x-full md:block">
      <div className="px-6 py-5 text-gray-800 font-bold text-sm uppercase bg-gray-100 border-b">
        {email ? email : "FRONTENDRH@GMAIL.COM"}
      </div>
      ...
      <nav className="mt-3">
        <ul className="space-y-3 px-4">
          ...
          {/* Dashboard */}
          {['/', '/Bookings', '/Cabs', '/Drivers', '/Complaints', '/Payments', '/Vendor'].map((path, index) => {
            // eslint-disable-next-line react/jsx-key
            const icons = [<FaTachometerAlt />, <FaCar />, <FaCar />, <FaUserFriends />, <FaExclamationTriangle />, <FaCreditCard />, <FaUser />];
            const labels = ['Dashboard', 'Bookings', 'Cabs', 'Drivers', 'Complaints', 'Payments', 'Vendor'];
            return (
              <li
                key={index}
                className="flex items-center gap-3 px-4 py-3 text-gray-700 rounded-lg cursor-pointer transition-all duration-300 hover:text-blue-600 hover:bg-blue-100"
              ...
              >
                {icons[index]}
                <span className="text-base font-medium">{labels[index]}</span>
              </li>
            );
          })}
